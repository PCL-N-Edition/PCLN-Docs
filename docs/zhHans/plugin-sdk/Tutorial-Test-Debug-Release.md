...
> 适用于 SDK `0.1.0-alpha.4`。目标是让同一个提交可重复通过测试、生成 `.pnp` 并发布。
...
dotnet add .\PclNToolbox.Plugin.Tests package PCLN.Plugin.Testing --version 0.1.0-alpha.4
...
1. 使用 Debug 或未签名 Release 包时，在 PCL N 完成开发者模式授权。
